...
export default function BookRdv() {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-4">Book RDV</h2>


      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <input value={childName} onChange={e=>setChildName(e.target.value)} placeholder="Child's name" className="bg-gray-700 px-2 py-1 rounded-md" />
        </div>
          ...
          {slots.filter(s=>!s.booked && !s.removed).map(slot => (
            <div key={slot.id} className="bg-gray-800 p-3 rounded-md">
              ...
              <div className="mt-2 flex gap-2">
                <button onClick={()=>handleBook(slot.id)} className="bg-green-600 px-2 py-1 rounded-md">Book</button>
              </div>
            </div>
          ))}
        </div>
